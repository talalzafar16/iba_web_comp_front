...
// @ts-expect-error jk kj
  const handleCreateCollection = (newCollection) => {
    setCollections([...collections, { ...newCollection, id: collections.length + 1 }]);
  };
            ...
            collections.map((collection) => (
              ...
              >
                <div className="relative w-full h-40 rounded-lg overflow-hidden">
                  ...
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} whileHover={{ opacity: 1 }} className="absolute hidden w-0 h-0 inset-0 bg-black/50 opacity-0 group-hover:opacity-100 group-hover:w-full group-hover:h-full group-hover:flex duration-700 items-center justify-center transition-all" > <Link to={`/dashboard/collection/${collection.id}`} className="text-white text-lg cursor-pointer font-semibold"> View Collection </Link> </motion.div>
